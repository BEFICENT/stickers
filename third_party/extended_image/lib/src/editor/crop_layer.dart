...
class ExtendedImageCropLayerState extends State<ExtendedImageCropLayer>
  ...
  void moveUpdate(_MoveType moveType, Offset delta) {
    ...
    if (_currentMoveType == null) {
      _currentMoveType = moveType;
      _moveStartCropRect = cropRect;
    ...
    final Rect previousCropRect = _moveStartCropRect!;
    Rect? result = previousCropRect;
    ...
    result =
        widget.editorConfig.clampCropRectToImage
            ? _clampCropRect(previousCropRect, result, cropRectLayoutRect!)
            : widget.editActionDetails.updateCropRect(result);

    if (result.beyond(layoutRect) && widget.editorConfig.autoCenterCropRect) {
      cropRect = result;
  ...
  Rect _clampCropRect(Rect current, Rect desired, Rect layoutBounds) {
    if (_cropRectFits(desired, layoutBounds)) {
      return desired;
    }

    // Interpolating from the current rectangle preserves the opposite corner
    // (or edge and center for side handles). Find the furthest valid position
    // rather than replacing the drag with the library's recentered rectangle.
    double valid = 0;
    double invalid = 1;
    for (int i = 0; i < 16; i++) {
      final double candidateT = (valid + invalid) / 2;
      final Rect candidate = Rect.lerp(current, desired, candidateT)!;
      if (_cropRectFits(candidate, layoutBounds)) {
        valid = candidateT;
      } else {
        invalid = candidateT;
      }
    }
    return Rect.lerp(current, desired, valid)!;
  }

  bool _cropRectFits(Rect candidate, Rect layoutBounds) {
    if (candidate.left < layoutBounds.left ||
        candidate.top < layoutBounds.top ||
        candidate.right > layoutBounds.right ||
        candidate.bottom > layoutBounds.bottom) {
      return false;
    }

    final Rect imageClamped = widget.editActionDetails.updateCropRect(
      candidate,
    );
    const double tolerance = 0.01;
    return (imageClamped.left - candidate.left).abs() < tolerance &&
        (imageClamped.top - candidate.top).abs() < tolerance &&
        (imageClamped.right - candidate.right).abs() < tolerance &&
        (imageClamped.bottom - candidate.bottom).abs() < tolerance;
  }
  ...
  void _moveEnd(_MoveType moveType) {
    if (_currentMoveType != null && moveType == _currentMoveType) {
      _resetMove();
      if (widget.editorConfig.autoCenterCropRect) {
        _startTimer();
      }
    }
  }
  ...
  void _startTimer() {
    ...
    _timer = Timer.periodic(widget.editorConfig.tickerDuration, (Timer timer) {
      ...
      final RectTween tween = RectTween(
        begin: oldScreenCropRect,
        end: newScreenCropRect,
      );
